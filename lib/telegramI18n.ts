...
const ru = {
  ...
  'order.pickProduct': 'Какой товар для «{line}»? Выберите:',
  'order.pickColor': '<b>{product}</b>\nВыберите цвет для «{line}»:',
  'order.pickSize': '<b>{product}, {color}</b>\nВыберите размер для «{line}» — нужно {qty} шт. Остатки на складе:',
...
const uz: Record<MessageKey, string> = {
  ...
  'order.pickProduct': "«{line}» uchun qaysi tovar? Tanlang:",
  'order.pickColor': "<b>{product}</b>\n«{line}» uchun rangni tanlang:",
  'order.pickSize': "<b>{product}, {color}</b>\n«{line}» uchun o'lchamni tanlang — {qty} dona kerak. Ombordagi qoldiq:",
